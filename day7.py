...
def score_hand(hand):
    counts = list(sorted(Counter(hand).values(), reverse=True))
    if counts == [5]: return 1
    if counts == [4, 1]: return 2
    if counts == [3, 2]: return 3
    if counts == [3, 1, 1]: return 4
    if counts == [2, 2, 1]: return 5
    if counts == [2, 1, 1, 1]: return 6
    if counts == [1, 1, 1, 1, 1]: return 7
